...
@section('js')
    ...
    <script>
        var ticketUrl = "{{ route('creditoventa.ticket', ['id' => ':id']) }}";
        var abonosUrl = "{{ route('creditoventa.abonos', ['id' => ':id']) }}";
        var detalleUrl = "{{ route('creditoventa.detalle', ['id' => ':id']) }}";
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return `<a class="btn btn-sm btn-danger" href="${abonosUrl.replace(':id', row.id)}">Abonos</a>` +
                                abonarButton;
                        }
                    }
                ],
                            ...
                            fetch("{{ route('creditoventa.registrarAbono') }}", {
                                ...
                                .then(data => {
                                    ...
                                    if (data.icon === 'success') {
                                        ...
                                        setTimeout(() => {
                                            window.open(
                                                `${ticketUrl.replace(':id', data.ticket)}`,
                                                '_blank');
                                        }, 1500);
                                    }
                                })
        ...
        function abonoCredito(creditoId) {
            fetch(detalleUrl.replace(':id', creditoId), {
                    method: 'GET',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json'
